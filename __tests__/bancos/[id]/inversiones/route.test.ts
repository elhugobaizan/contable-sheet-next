...
import { GET, POST } from '@/app/bancos/[id]/inversiones/route';
...
describe('Bancos API - Route /bancos/[id]/inversiones', () => {
  ...
  describe('GET /bancos/[id]/inversiones', () => {
    it('debe listar las inversiones del banco exitosamente', async () => {
      ...
      const response = await GET(new NextRequest('http://localhost:3000/api/bancos/123/inversiones'), { params });
    ...
    it('debe retornar 400 cuando el ID de banco no es proporcionado', async () => {
      const params = Promise.resolve({ id: '' });
      const response = await GET(new NextRequest('http://localhost:3000/api/bancos/inversiones'), { params });
      const data = await response.json();
    ...
    it('debe manejar errores al listar inversiones', async () => {
      ...
      expect(data.error).toBe(`Error al listar inversiones del banco ${mockBancoId}`);
    });
  });

  describe('POST /bancos/[id]/inversiones', () => {
    it('debe crear una inversión para el banco exitosamente', async () => {
      const inversionData = {
      ...
      const params = Promise.resolve({ id: mockBancoId });
      const req = new NextRequest('http://localhost:3000/api/bancos/123/inversiones', {
        method: 'POST',
    ...
    it('debe retornar 400 cuando el ID de banco no es proporcionado', async () => {
      ...
      const params = Promise.resolve({ id: '' });
      const req = new NextRequest('http://localhost:3000/api/bancos/inversiones', {
        method: 'POST',
        body: JSON.stringify(inversionData),
        headers: { 'Content-Type': 'application/json' },
      });
    ...
    it('debe manejar errores al crear inversión', async () => {
      ...
      expect(response.status).toBe(500);
      expect(data.error).toBe(`Error al crear inversion(es) para el banco ${mockBancoId}`);
