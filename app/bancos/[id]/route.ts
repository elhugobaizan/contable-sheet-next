...
export async function GET(req: Req, { params }: { params: Promise<{ id: string }> }) {
    ...
    try {  
        ...
        const result = {obj: `await prisma.banco.findMany({where: { id: ${id} }})`};
...
export async function UPDATE(req: Req, { params }: { params: Promise<{ id: string }> }) {
...
export async function PUT(req: Req, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
      const id = (await params).id;
      const result = {obj: `await prisma.banco.delete({
          where: {
              id: id
          }
      })`};
